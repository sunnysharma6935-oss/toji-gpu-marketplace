...
func Signup(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusCreated, map[string]string{"user_id": id.String(), "token": token})
}
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]string{"user_id": id, "token": token})
}
...
func RegisterHost(w http.ResponseWriter, r *http.Request) {
	...
	_, err := db.Pool.Exec(r.Context(),
		...
		hostID, userID, req.MachineLabel, agentToken,
	)
	if err != nil {
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
...
func HostHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Pool.Exec(
		r.Context(),
		`UPDATE hosts SET last_heartbeat_at = NOW(), status = 'online' WHERE agent_token = $1`,
		token,
	)
	if err != nil {
...
func GPUReport(w http.ResponseWriter, r *http.Request) {
	...
	var hostID string
	err := db.Pool.QueryRow(r.Context(), `SELECT id FROM hosts WHERE agent_token = $1`, token).Scan(&hostID)
	if err != nil {
		http.Error(w, "invalid agent token", http.StatusUnauthorized)
		return
	}
	...
	_, err = db.Pool.Exec(
		r.Context(),
		`INSERT INTO gpus (id, host_id, gpu_index, model, vram_mb, driver_version)
		 VALUES ($1, $2, $3, $4, $5, $6)
		 ON CONFLICT (host_id, gpu_index)
		 DO UPDATE SET model = EXCLUDED.model, vram_mb = EXCLUDED.vram_mb, driver_version = EXCLUDED.driver_version`,
		uuid.New(), hostID, req.GPUIndex, req.Model, req.VRAMMB, req.DriverVersion,
	)
	if err != nil {
...
func CreateListing(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Pool.QueryRow(
		r.Context(),
		`SELECT g.id FROM gpus g JOIN hosts h ON h.id = g.host_id WHERE g.id = $1 AND h.user_id = $2`,
		req.GPUID, userID,
	).Scan(&gpuID)
	if err != nil {
	...
	_, err = db.Pool.Exec(
		r.Context(),
		`INSERT INTO listings (id, gpu_id, price_paise_per_hour, is_active) VALUES ($1, $2, $3, true)`,
		listingID, gpuID, req.PricePaiseHour,
	)
	if err != nil {
...
func ListActiveListings(w http.ResponseWriter, r *http.Request) {
	rows, err := db.Pool.Query(
		r.Context(),
		`SELECT l.id, g.id, g.model, g.vram_mb, g.driver_version, h.machine_label, l.price_paise_per_hour
		 FROM listings l
		 JOIN gpus g ON g.id = l.gpu_id
		 JOIN hosts h ON h.id = g.host_id
		 WHERE l.is_active = true AND h.status = 'online'
	...
	for rows.Next() {
		var item Listing
		if err := rows.Scan(&item.ListingID, &item.GPUID, &item.Model, &item.VRAMMB,
			&item.DriverVersion, &item.MachineLabel, &item.PricePaisePerHour); err != nil {
...
// CreateRental — MODIFIED. Now reserves the customer's TOJI balance in the SAME
// transaction as creating the rental row: if the reservation fails, nothing about
// this rental exists at all, not even in 'provisioning'. This is the corrected
// ordering from the approved design (reservation moved earlier than the running
// transition, so a container is never provisioned for a customer who can't afford it).
func CreateRental(w http.ResponseWriter, r *http.Request) {
		...
		`SELECT l.id, l.price_paise_per_hour
		 ...
		 WHERE l.id = $1
		   ...
		   AND NOT EXISTS (
		       ...
		       WHERE r.listing_id = l.id AND r.status IN ('provisioning', 'running')
		   )`,
		req.ListingID,
	...
	if err := billing.ReserveAtRentalCreation(r.Context(), tx, custAccountID, rentalID, ratePaisePerHour); err != nil {
...
func GetPendingRentals(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Pool.Query(
		...
		`SELECT r.id, g.gpu_index
		 ...
		 JOIN gpus g ON g.id = l.gpu_id
		 WHERE g.host_id = $1 AND r.status = 'provisioning'`,
		hostID,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.RentalID, &item.GPUIndex); err != nil {
